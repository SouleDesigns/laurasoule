...
jQuery(window).load(function () {
	//Preloader
	setTimeout("jQuery('#preloader').animate({'opacity' : '0'},300,function(){jQuery('#preloader').hide()})",200);
	setTimeout("jQuery('.preloader_hide').animate({'opacity' : '1'},500)",200);
	...
	$.getJSON("http://picasaweb.google.com/data/feed/api/user/lauraksoule?kind=photo&max-results=9&alt=json", function(data){
		$.each(data.feed.entry, function(i,entry){
			if(i<=8){ // <— change this number to display more or less images
				...
				.wrap("<li><a href='" + entry.link[2].href + "' target='_blank' title='Google+'></a></li>");
			}
		});			
	});
});
