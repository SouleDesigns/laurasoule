/**
 * Created by osoule on 12/2/14.
 */
/**
 * Google Plus jQuery Photo Gallery
 * This library was inspired aon pwa by Jeroen Diderik
 * @name jquery.gplus.js
 * @author Alin Oancea - http://www.alinoancea.es/
 * @revision 2.1.0
 * @date may 20, 2013
 * @copyright (c) 2013 AlinOancea(www.alinoancea.es)
 * @license Creative Commons Attribution-Share Alike 3.0 Netherlands License - http://creativecommons.org/licenses/by-sa/3.0/nl/
 */
(function ($) {
    var elem, opts = {};
    $.fn.gplus = function (opts) {
        var $self, settings = {};
        opts = $.extend({}, $.fn.gplus.defaults, opts);
        elem = this;
        function _initialize() {
            settings = opts;
            ts = new Date().getTime();
            settings.id = ts;
            $self = $("<div id='gplus_" + ts + "'/>").appendTo(elem);
            $self.addClass('gplus_container');
            _start();
            return false;
        }
        function _start() {
            if (settings.username === '') {
                alert('Make sure you specify at least your username.' + '\n' + 'See info documents for more info');
                return;
            }
        ...
        function photo(j) {
            ...
            $html = $("<div class='gplus_photo' style='height:" + (settings.thumbSize + 1) + "px;cursor: pointer;'/>");
            ...
            if (typeof (settings.onclickThumb) === "function") { var obj = j; $html.bind('click.gplus', obj, clickThumb); }
        ...
        function albums(j) {
            ...
            while (i < settings.albumMaxResults && i < $na && i < (settings.albumsPerPage * settings.albumPage)) {
                ...
                if ($.inArray($id_base, settings.albums) > -1 || settings.albums.length === 0) {
                    $scAlbum = $("<div class='gplus_album'/>");
                    var jfeed = j.feed.entry[i];
                    $scAlbum.bind('click.gplus', jfeed, function (e) {
                        e.stopPropagation();
                        settings.page = 1;
                    ...
                    if (settings.showAlbumTitles) {
                        $scAlbum.append("<p>" + j.feed.entry[i].title.$t + "<p/>" + (settings.showAlbumdate ? $album_date : "") + (settings.showAlbumPhotoCount ? "&nbsp;&nbsp;&nbsp;&nbsp;" + j.feed.entry[i].gphoto$numphotos.$t + " " + settings.labels.photos : ""));
                    }
            ...
            if ($na > settings.albumsPerPage) {
                ...
                var $ppage = $("<div class='gplus_prevpage'/>").text(settings.labels.prev),
                    $npage = $("<div class='gplus_nextpage'/>").text(settings.labels.next);
                $navRow = $("<div class='gplus_pager'/>");
                if (settings.albumPage > 1) {
                    $ppage.addClass('link').bind('click.gplus', function (e) {
                        e.stopPropagation();
                ...
                for (var p = 1; p < $pageCount + 1; p++) {
                    if (p == settings.albumPage) {
                        tmp = "<div class='gplus_pager_current'>" + p + "</div> ";
                    } else {
                        tmp = $("<div class='gplus_pager_page'>" + p + "</div>").bind('click.gplus', p, function (e) {
                            e.stopPropagation();
                ...
                if (settings.albumPage < $pageCount) {
                    $npage.addClass('link').bind('click.gplus', function (e) {
                        e.stopPropagation();
                        settings.albumPage = (parseInt(settings.albumPage, 10) + 1);
                        albums(j);
                        return false;
        ...
        function album(j) {
            ...
            if (settings.mode != 'album' && settings.mode != 'keyword') {
                tmp = $("<div class='gplus_album_backlink'>" + settings.labels.albums + "</div>").bind('click.gplus', function (e) {
                    e.stopPropagation();
                    getAlbums();
                    return false;
                });
            ...
            if (settings.showAlbumDescription) {
                $scPhotosDesc = $("<div class='gplus_album_description'/>");
            ...
            var $s = $(".gplus_photo", $scPhotos).css(settings.thumbCss);
        ...
        function latest(j) {
            ...
            var $s = $("div.gplus_photo", $scPhotos).css(settings.thumbCss);
            if (typeof (settings.popupExt) === "function") {
                settings.popupExt($s.find("a[rel='lb-" + settings.username + "']"));
            } else if (typeof (settings.onclickThumb) != "function" && $.slimbox) {
                $s.find("a[rel='lb-" + settings.username + "']").slimbox(settings.slimbox_config);
    ...
    $.fn.gplus.defaults = {
        ...
        thumbSize: 160,  //-- specify thumbnail size of photos (default: 72, cropped not supported, supported cropped/uncropped: 32, 48, 64, 160 and uncropped only: 72, 144, 200, 288, 320, 400, 512, 576, 640, 720, 800)
        ...
        labels: {
            photo: "photo",
            photos: "photos",
            albums: "<img src='./images/indicate.png'/> &nbsp; &nbsp;back to gallery",
        ...
        blockUIConfig: {
            message: "<div class='lbLoading gplus_loader'>Loading...</div>",
            css: "gplus_loader"
